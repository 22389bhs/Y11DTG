...
def print_all_movie_by_oscars_won():
    '''print all the movies by the ammount of the oscars the movie has won ordering the information either by descendingo ascending'''
    with sqlite3.connect(DATABASE) as db:
        cursor = db.cursor()
        sql = 'SELECT * from movies ORDER BY oscars_won;'
        cursor.execute(sql)
        results = cursor.fetchall()
        #loop through all the results then
        #print them out nicely

        inp = user_input = ("Do you want the information to be printed out by descending or ascending? ")
        if inp == "descending":
            for movie in results:
         print(f"{'Oscars Won':<20} {'Movie':<30} ")
              elif inp == "ascending":
            print(f"{movie[7]<15} {movie[0]<30}")
